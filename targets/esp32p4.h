/**
 ******************************************************************************
 * @file           : esp32p4.h
 * @brief          : Declares ESP32-P4 target support.
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
 * Copyright (c) 2026 Kuraga Tech
 * SPDX-License-Identifier: MIT
 *
 ******************************************************************************
 * @details
 *
 * Defines target metadata, flasher stubs, register addresses, and chip
 * information callbacks for ESP32-P4 based devices.
 *
 * Features:
 * - ESP32-P4 ROM detection and target naming helpers
 * - Embedded RAM stub text and data payloads
 * - SPI flash, efuse, and crystal-frequency hooks
 *
 * Usage Example:
 * ```cpp
 * Esp32P4 target(nullptr);
 * QString name = target.CHIP_NAME();
 * ```
 *
 ******************************************************************************
 */

#ifndef ESP_32P4_H
#define ESP_32P4_H

#include <QObject>
#include <QString>
#include <QByteArray>
#include "esp_base.h"

class Esp32P4 : public EspBase
{
    Q_OBJECT
private:
    // This ROM address has a different value on each chip model
    virtual uint32_t CHIP_DETECT_MAGIC_VALUE() {return 0x0;}

public:
    explicit Esp32P4(QObject *parent) : EspBase {parent} {}
    QString CHIP_NAME() {return "ESP32-P4";}
    QVector<QString> CHIP_TARGETS() {return {CHIP_NAME()};};

    // STUB
    virtual uint32_t stub_entry() {return 1341195718;}
    virtual std::vector<uint8_t> stub_text() {return ba_to_vec(QByteArray::fromBase64(""));}
    virtual uint32_t stub_text_start() {return 1341194240;}
    virtual std::vector<uint8_t> stub_data() {return ba_to_vec(QByteArray::fromBase64("DAD1T+4I8U86CfFPkgnxT2gK8U/UCvFPggrxT7YH8U8kCvFPZArxT6YJ8U9mB/FP2gnxT2YH8U/ICPFPDAnxTzoJ8U+SCfFP2gjxTyAI8U9QCPFP1gjxTywN8U86CfFP7AvxT+AM8U+yBvFPCg3xT7IG8U+yBvFPsgbxT7IG8U+yBvFPsgbxT7IG8U+yBvFPiAvxT7IG8U8IDPFP4AzxTw=="));}
    virtual uint32_t stub_data_start() {return 1341533096;}

    // SPI
    virtual uint32_t SPI_REG_BASE() {return 0x5008D000;}
    virtual uint32_t SPI_USR_REG()  {return SPI_REG_BASE() + 0x18;}
    virtual uint32_t SPI_USR1_REG() {return SPI_REG_BASE() + 0x1C;}
    virtual uint32_t SPI_USR2_REG() {return SPI_REG_BASE() + 0x20;}
    virtual uint32_t SPI_W0_REG()   {return SPI_REG_BASE() + 0x58;}
    virtual bool     SPI_MOSI_DLEN_OFFS() {return true;}
    virtual uint32_t SPI_MOSI_DLEN_REG()  {return SPI_REG_BASE() + 0x24;}
    virtual uint32_t SPI_MISO_DLEN_REG()  {return SPI_REG_BASE() + 0x28;}

    // efuse
    virtual uint32_t EFUSE_RD_REG_BASE() {return 0x5012D000 + 0x030;}

    // functions
    virtual bool get_chip_description(QString* description, void* esp_tool);
    virtual bool get_chip_features(QString* features, void* esp_tool);
    virtual uint32_t get_crystal_freq(void* esp_tool);

};

#endif // ESP_32P4_H
