/**
 ******************************************************************************
 * @file           : esp32c3.h
 * @brief          : Declares ESP32-C3 target support.
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
 * Copyright (c) 2026 Kuraga Tech
 * SPDX-License-Identifier: MIT
 *
 ******************************************************************************
 * @details
 *
 * Defines target metadata, flasher stubs, register addresses, and chip
 * information callbacks for ESP32-C3 based devices.
 *
 * Features:
 * - ESP32-C3 ROM detection and supported module names
 * - Embedded RAM stub text and data payloads
 * - SPI flash, efuse, and fixed crystal hooks
 *
 * Usage Example:
 * ```cpp
 * Esp32C3 target(nullptr);
 * QString name = target.CHIP_NAME();
 * ```
 *
 ******************************************************************************
 */

#ifndef ESP_32C3_H
#define ESP_32C3_H

#include <QObject>
#include <QString>
#include <QByteArray>
#include "esp_base.h"

class Esp32C3 : public EspBase
{
    Q_OBJECT
protected:
    uint32_t CHIP_DETECT_MAGIC_VALUE() {return 0x0;} // unused in ESP32C3

public:
    explicit Esp32C3(QObject *parent) : EspBase {parent} {}
    QString CHIP_NAME() {return "ESP32-C3";}
    QVector<QString> CHIP_TARGETS();

    // This ROM address has a different value on each chip model
    virtual bool CHIP_COMPARE_MAGIC_VALUE(uint32_t x) { 
	    std::vector<uint32_t> CHIP_DETECT_MAGIC_VALUE = {0x6921506F, 0x1B31506F, 0x4881606F, 0x4361606F};
		for (uint32_t magic : CHIP_DETECT_MAGIC_VALUE) {
			if (magic == x) return true;
		}
		return false;
	}

    // STUB
    virtual uint32_t stub_entry() {return 1077413532;}
    virtual std::vector<uint8_t> stub_text() {return ba_to_vec(QByteArray::fromBase64(""));}
    virtual uint32_t stub_text_start() {return 1077411840;}
    virtual std::vector<uint8_t> stub_data() {return ba_to_vec(QByteArray::fromBase64("FEDIP4wKOEDcCjhANAs4QNgLOEBEDDhA8gs4QD4JOECUCzhA1As4QEgLOEDuCDhAfAs4QO4IOEBmCjhArAo4QNwKOEA0CzhAeAo4QKIJOEDsCThAdAo4QJwOOEDcCjhAXA04QFQOOEAuCDhAfA44QC4IOEAuCDhALgg4QC4IOEAuCDhALgg4QC4IOEAuCDhA+Aw4QC4IOEB6DThAVA44QA=="));}
    virtual uint32_t stub_data_start() {return 1070164912;}

    // SPI
    virtual uint32_t SPI_REG_BASE() {return 0x60002000;}
    virtual uint32_t SPI_USR_REG()  {return SPI_REG_BASE() + 0x18;}
    virtual uint32_t SPI_USR1_REG() {return SPI_REG_BASE() + 0x1C;}
    virtual uint32_t SPI_USR2_REG() {return SPI_REG_BASE() + 0x20;}
    virtual uint32_t SPI_W0_REG()   {return SPI_REG_BASE() + 0x58;}
    virtual bool     SPI_MOSI_DLEN_OFFS() {return true;}
    virtual uint32_t SPI_MOSI_DLEN_REG()  {return SPI_REG_BASE() + 0x24;}
    virtual uint32_t SPI_MISO_DLEN_REG()  {return SPI_REG_BASE() + 0x28;}

    // efuse
    virtual uint32_t EFUSE_RD_REG_BASE()  {return 0x60008800 + 0x030;}

    // functions
    virtual bool get_chip_description(QString* description, void* esp_tool);
    virtual bool get_chip_features(QString* features, void* esp_tool);
    virtual uint32_t get_crystal_freq(void* esp_tool);

};

#endif // ESP_32C3_H
