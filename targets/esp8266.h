/**
 ******************************************************************************
 * @file           : esp8266.h
 * @brief          : Declares ESP8266 target support.
 * @author         : Kuraga Team
 ******************************************************************************
 * @attention
 *
 * Copyright (c) 2026 Kuraga Tech
 * SPDX-License-Identifier: MIT
 *
 ******************************************************************************
 * @details
 *
 * Defines target metadata, flasher stubs, register addresses, and chip
 * information callbacks for ESP8266 and ESP8285 based devices.
 *
 * Features:
 * - ESP8266 ROM detection and supported module names
 * - Embedded RAM stub text and data payloads
 * - SPI flash, efuse, and crystal-frequency hooks
 *
 * Usage Example:
 * ```cpp
 * Esp8266 target(nullptr);
 * QString name = target.CHIP_NAME();
 * ```
 *
 ******************************************************************************
 */

#ifndef ESP_8266_H
#define ESP_8266_H

#include <QObject>
#include <QString>
#include <QByteArray>
#include "esp_base.h"

class Esp8266 : public EspBase
{
    Q_OBJECT
private:
    virtual uint32_t CHIP_DETECT_MAGIC_VALUE() {return 0xFFF0C101;}

public:
    explicit Esp8266(QObject *parent) : EspBase {parent} {}
    QString CHIP_NAME() {return "ESP8266";}
    QVector<QString> CHIP_TARGETS();

    // STUB
    virtual uint32_t stub_entry() {return 1074843652;}
    virtual std::vector<uint8_t> stub_text() {return ba_to_vec(QByteArray::fromBase64(""));}
    virtual uint32_t stub_text_start() {return 1074843648;}
    virtual std::vector<uint8_t> stub_data() {return ba_to_vec(QByteArray::fromBase64(""));}
    virtual uint32_t stub_data_start() {return 1073720492;}

    // SPI
    virtual uint32_t SPI_REG_BASE() {return 0x60000200;}
    virtual uint32_t SPI_USR_REG()  {return SPI_REG_BASE() + 0x1C;}
    virtual uint32_t SPI_USR1_REG() {return SPI_REG_BASE() + 0x20;}
    virtual uint32_t SPI_USR2_REG() {return SPI_REG_BASE() + 0x24;}
    virtual uint32_t SPI_W0_REG()   {return SPI_REG_BASE() + 0x40;}
    virtual bool     SPI_MOSI_DLEN_OFFS() {return false;}
    virtual uint32_t SPI_MOSI_DLEN_REG() {return 0x00;} // NONE
    virtual uint32_t SPI_MISO_DLEN_REG() {return 0x00;} // NONE

    // efuse
    virtual uint32_t EFUSE_RD_REG_BASE() {return 0x00;} // NONE

    // xtal
    const uint32_t UART_CLKDIV_REG = 0x60000014;
    const uint32_t XTAL_CLK_DIVIDER = 2;
    const uint32_t UART_CLKDIV_MASK = 0xFFFFF;

    // functions
    virtual bool get_chip_description(QString* description, void* esp_tool);
    virtual bool get_chip_features(QString* features, void* esp_tool);
    virtual uint32_t get_crystal_freq(void* esp_tool);

};

#endif // ESP_8266_H
